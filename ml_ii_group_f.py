...
lr_scores_val = cross_val_score(lr_model, X_val, y_val, scoring='accuracy', cv=1)
print("Accuracy: %0.4f (+/- %0.2f)" % (np.median(lr_scores_val), np.std(lr_scores_val)))
plot_scores([lr_scores_val], ['LR val'])

xgb_scores_tunned_val = cross_val_score(xgb_model_after_search, X_val, y_val, scoring='accuracy', cv=1)
print("Accuracy: %0.4f (+/- %0.2f)" % (np.median(xgb_scores_tunned_val), np.std(xgb_scores_tunned_val)))
plot_scores([xgb_scores_tunned_val, lr_scores_val], ['XGB tunned val', 'LR val'])
